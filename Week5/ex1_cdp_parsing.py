...
for cdp_data_list in cdp_device_list:
    ...
    for data in cdp_data:
        ...
        if 'Platform: ' in data:
            network_devices[hostname]['vendor'] = data.split(None, 3)[1]
            network_devices[hostname]['model'] = data.split(None, 3)[2].strip(',')
            network_devices[hostname]['device_type'] = data.split(None, 5)[4]

        # Process IP
        if 'IP address: ' in data:
            network_devices[hostname]['ip'] = data.split(None, 2)[2]
